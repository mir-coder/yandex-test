...
class ItcSlider {
  ...
  #touchEnd(e) {
    ...
    const event = e.type.startsWith('touch') ? e.changedTouches[0] : e;
    const wrapperRect = this.#state.elWrapper.getBoundingClientRect();
    let clientX = Math.max(wrapperRect.left, Math.min(event.clientX, wrapperRect.right));
    ...
    if (!this.#config.loop) {
        ...
        if (isNotMoveFirst || isNotMoveLast) { // Заменил ' ' на '||'
            diffPosX = 0;
        }
    }
    ...
    if (value > this.constructor.#SWIPE_THRESHOLD) {
        this.#state.direction = 'next';
        let count = Math.floor(Math.abs(value) - this.constructor.#SWIPE_THRESHOLD) / 100;
...
#touchMove(e) {
    ...
    const prevPosX = this.#state.prevPosX || event.clientX;
    const direction = prevPosX > event.clientX ? 'next' : 'prev';
    this.#state.prevPosX = event.clientX;

    if (!this.#state.isTouchMoving) {
        if (Math.abs(diffPosY) > Math.abs(diffPosX) || Math.abs(diffPosX) === 0) { // Заменил ' ' на '||'
            this.#state.isSwiping = false;
            return;
        }
    ...
    if (!this.#config.loop) {
        const isNotMoveFirst = this.#state.activeItems[0] === 1 && diffPosX <= 0;
        const isNotMoveLast = this.#state.activeItems[this.#state.activeItems.length - 1] && diffPosX >= 0;
        // Логика, связанная с isNotMoveFirst и isNotMoveLast, может быть добавлена здесь
    }
}
